...
public class ExperimentDifferentParametersOOB {

	static MainTask currentTask = new EvaluatePrequential();
	static Writer writer; 
	
	public ExperimentDifferentParametersOOB(){
			...
			for(int i = 0; i < ensembleSizes.length; i++){
				for(int j = 0; j < theta.length; j++){
					...
					for(int r = 0; r < 5; r++){
						...
						String task = "EvaluatePrequential -i 2500 -l (meta.WaitForLabelsOOB -s " + ensembleSizes[i]
								+ " -t "+theta[j]+")  -s  (ArffFileStream -f ("+path+"arffs/"
								+ datasets[d] + ".arff) -c 15) -e (FadingFactorEachClassPerformanceEvaluator -a 0.99)"
										+ " -f 1 -d "+path+"RQ3/ParamEval/oob/"+datasets[d]+"/"
